...
void main() {
  ...
  test('screening accuracy scales repetitions and hint support', () async {
    ...
    expect(await supported.buildTargets(), hasLength(5));
    expect(independent.difficulty, GameDifficulty.independent);
    expect(independent.hintLevel, GameHintLevel.minimal);
    expect(await independent.buildTargets(), hasLength(3));
  });

  test('cluster evidence receives a build-and-sequence route', () async {
    ...
    final targets = await config.buildTargets();
    expect(
      targets.map((target) => target.promptText),
